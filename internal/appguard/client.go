...
func NewClient(profileName string, debug bool) (*Client, error) {
	...
	if profile.AppGuardAppKey == "" {
		return nil, fmt.Errorf("AppGuard AppKey가 설정되지 않았습니다. 'nhn configure'로 설정하세요")
	}
	...
	return &Client{
		...
		baseURL:    "https://appguard.api.nhncloudservice.com",
		appKey:     profile.AppGuardAppKey,
		headers:    headers,
		debug:      debug,
	}, nil
}
